...
export function ImageSourceToggleField(props: CheckboxFieldClientProps) {
  const {
    field: {
  ...
  const usesUpload = Boolean(value)
    ...
    <div className={['field-type', 'checkbox', className, showError && 'error', isDisabled && 'read-only'].filter(Boolean).join(' ')}>
      ...
      <div className={'field-type__wrap'}>
        ...
        >
          <span
            style={{
              background: !usesUpload ? 'var(--theme-elevation-0)' : 'transparent',
              border: !usesUpload ? '1px solid var(--theme-elevation-150)' : '1px solid transparent',
              borderRadius: 999,
              color: !usesUpload ? 'var(--theme-text)' : 'var(--theme-elevation-500)',
